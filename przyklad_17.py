...
class Vektor:
    ...
    # --- nowa funkcjonalność formatowania w przestrzeniach N-wymiarowych ---
    def angle(self, n):
        # zwraca N-wymiarową „hiper-odchyłkę” względem osi n-1
        # w 2D -> atan2(y, x)
        # w 3D -> pierwszy kąt sferyczny
        # w ND -> kąt między kolejnymi "poziomami" projekcji
        r = math.sqrt(sum(x ** 2 for x in self[n:]))
        a = math.atan2(r, self[n-1])
        # to jest poprawne: ostatnia "półoś" w N wymiarach
    ...
    def angles(self):
        # generator wszystkich kątów hiperbiegunowych
        # (bez wielkiego zużycia pamięci - nie zwraca listy)
        return (self.angle(n) for n in range(1, len(self)))

    def __format__(self, fcode=''):
        # standard: formatuje elementy (jak tuple)
        # sufiks 'h' = format hiperbiegunowy
        # np format(v, 'h') -> <|v|, psi1, psi2, ...>
        if fcode.endswith('h'):
            fcode = fcode[:-1]
